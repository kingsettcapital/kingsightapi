...
public sealed class FabricWarehouseTables
{
    ...
    /// <summary>Gold warehouse catalog (<c>FabricWarehouse:Database</c>).</summary>
    ...
    /// <summary>Portal mortgage tables in <c>{Database}.mort</c> (CMHC upload history, etc.).</summary>
    public string PortalMort(string table) => $"{Database}.mort.{table}";

    /// <summary>Portal auth tables in <c>{Database}.input</c> (legacy UserMst, RoleMst).</summary>
    public string PortalInput(string table) => $"{Database}.input.{table}";

    /// <summary>User Management in <c>{Database}.subjective_input</c> (user_master, role_master).</summary>
    public string SubjectiveInput(string table) =>
        $"{Database}.subjective_input.{table}";

    /// <summary>Shared dimension tables in <c>{Database}.shared</c>.</summary>
    public string Shared(string table) =>
        $"{Database}.shared.{table}";

    /// <summary>Mortgage dimension tables in <c>{Database}.mortgage</c>.</summary>
    public string Mortgage(string table) =>
        $"{Database}.mortgage.{table}";

    /// <summary>Mortgage TVF/view-qualified name, e.g. <c>wh_gold.mortgage.fn_exposure</c>.</summary>
    public string MortgageObject(string name) =>
        $"{Database}.mortgage.{name}";
